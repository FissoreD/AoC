let test_day y d p1 p2 =
  ...
  let t1, s1 = Utils.time_it (fun _ -> p1 (Misc.Read_file.read_lines_string f_name)) in
  let t2, s2 = Utils.time_it (fun _ -> p2 (Misc.Read_file.read_lines_string f_name)) in
