let parse_line l =
  ...
  in
  List.map split_str l

let update_dir dir rot = (4 + if rot = 'R' then dir + 1 else dir - 1) mod 4

let update_pos (x, y) dir dist =
  if dir = 0 then (x + dist, y)
  else if dir = 1 then (x, y + dist)
  else if dir = 2 then (x - dist, y)
  else (x, y - dist)
...
let rec walk seen l dir pos dist =
  if List.mem pos seen then pos
  else if dist = 0 then update_seen seen (pos, dir) l
  else
    let pos' = update_pos pos dir 1 in
    walk (pos :: seen) l dir pos' (dist - 1)

and update_seen seen (pos, dir) l =
  if List.mem pos seen then pos
  else
    match l with
    | [] -> Utils.error 2016 1 "no pos visisted twice"
