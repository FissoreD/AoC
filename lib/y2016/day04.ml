...
let count_max l =
  ...
  let cnt = List.map (fun x -> (x, Misc.Utils.count x l)) s in
  let compare (x1, x2) (y1, y2) =
    let l = (compare x2 y2) * -1 in
    if l = 0 then String.compare x1 y1 else l in
...
let next l =
  ...
  let next_char (c: char) =
    if c = '-' then '-' else
    char_of_int ((int_of_char c - a + 1) mod 26 + a) in
  init (length l) (fun i -> next_char l.[i])

let rec rotate (l,x) = function
  | 0 -> l,x
