module SL = struct
  ...
  let rec compare (a : t) (b : t) : int =
    ...
    | _, [] -> -1
    | x :: xs, y :: ys -> (
        match String.compare x y with 0 -> compare xs ys | n -> n)
end
...
let parse l =
  ...
  let add k v =
    m := Map.add k (v :: (try Map.find k !m with Not_found -> [])) !m
  in
  let l2tuple l =
...
let rec invole_aux add pref l k v =
  match l with
  | [] -> ()
  ...
  | hd :: tl as l ->
      ...
      if List.length v <= List.length tl then invole_aux add (hd :: pref) tl k v

let involve add keys values l = Array.iter2 (invole_aux add [] l) keys values
...
let p2 l =
  ...
  let count_Rn_Ar l = List.(count "Rn" l + count "Ar" l) in
  let count_Y = List.count "Y" in
  let target, _ = parse (List.rev l) in
  List.length target - count_Rn_Ar target - (2 * count_Y target) - 1
  |> string_of_int
