...
let armor =  [ 0,0,0; 13,0,1; 31,0,2; 53,0,3; 75,0,4; 102,0,5] (*6*)
...
let rec run_game isP p1 p2 =
  ...
  else if p2.hp <= 0 then true else
  if isP then run_game (not isP) p1 {p2 with hp=p2.hp-p1.dmg+p2.arm}
...
let choose_items hp =
  let add (c2, pt) (c,d,a) = c+c2, {hp=hp; arm=pt.arm+a; dmg=pt.dmg+d} in
  ...
  let rec next_gen_l f1 l st = function
    | [] -> []  | hd :: tl -> next_gen f1 (next_gen_l f1 l) (l tl) st hd tl in
